...
export default function CommandCenterPage() {
  ...
  const atRisk = allWork.filter(
    (w) => w.status === "blocked" || (w.delayMinutes ?? 0) > 0,
  ).length;
            ...
            {open.map((issue) => (
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-2">
                  {issue.recommendation.action.status ===
                  "pending_approval" ? (
                    <Button
                      size="sm"
                      variant="primary"
                      onClick={() => approveAction(issue.id)}
                    >
                      Approve fix
                    </Button>
                  ) : (
                    <Badge tone="accent">
                      {issue.recommendation.action.status.replace(
                        "_",
                        " ",
                      )}
                    </Badge>
      ...
      <Card className="mt-6">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {issues.map((issue) => (
            <div
              key={issue.id}
              className="rounded-md border border-border bg-white/[0.02] p-4"
            >
              <div className="text-xs font-medium text-muted">
                {issue.recommendation.title}
              </div>

              <div className="mt-1.5 line-clamp-2 text-[11px] leading-relaxed text-muted/80">
                {issue.recommendation.reason}
              </div>

              <div className="mt-3 flex items-center justify-between">
                <Badge tone="good">
                  {Math.round(issue.recommendation.confidence * 100)}%
                  confidence
                </Badge>
          ...
          {issues.length === 0 && (
            <div className="col-span-full rounded-md border border-dashed border-border p-6 text-center text-xs text-muted">
